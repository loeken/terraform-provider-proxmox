...
local-dev-install: build
	...
	rm -rf ~/.terraform.d/plugins/localhost/loeken/proxmox
	mkdir -p ~/.terraform.d/plugins/localhost/loeken/proxmox/$(MAJOR).$(MINOR).$(NEXT_MICRO)/$(KERNEL)_$(ARCH)/
	cp bin/terraform-provider-proxmox ~/.terraform.d/plugins/localhost/loeken/proxmox/$(MAJOR).$(MINOR).$(NEXT_MICRO)/$(KERNEL)_$(ARCH)/
